 ...
 */
public class AugmentedImageRendererSingle {
    ...
    private final ObjectRenderer image = new ObjectRenderer();

    public AugmentedImageRendererSingle() {
    }

    public void createOnGlThread(Context context) throws IOException {
        image.createOnGlThread(context, "models/coca_cola_zero.obj", "models/coca_cola_zero.jpg");
...
//        final float mazeEdgeSize = 2.0F; // coca_cola_zero obj
//        final float mazeEdgeSize = 0.65F; // tire_front obj
        final float mazeEdgeSize = 1.0F; // Questo numero è calcolato cliccando "N" in blender e prendendo il valore più grande tra X,Y e Z.
        final float maxImageEdgeSize = Math.max(augmentedImage.getExtentX(), augmentedImage.getExtentZ()); // Get largest detected image edge size

        float scaleFactor = maxImageEdgeSize / mazeEdgeSize;
